...
USE_DEPTH = False  # Global flag to control depth estimation
...
def load_colmap_depth(datadir, factor=8, bd_factor=.75):
    if not USE_DEPTH:
        return []  # Return empty list when depth estimation is disabled
        
    data_file = datadir + '/colmap_depth.npy'
    if os.path.exists(data_file):
        print("Loading precomputed depth data...")
        return np.load(data_file, allow_pickle=True)
        
    print("Computing depth data (can be disabled with use_depth=False)...")
...
class LLFFDataset(Dataset):
    def __init__(self, datadir, split='train', downsample=4, is_stack=False, hold_every=8, frame_num=[], use_depth=False):
        """
        spheric_poses: whether the images are taken in a spheric inward-facing manner
                       default: False (forward-facing)
        val_num: number of val images (used for multigpu training, validate same image for all gpus)
        use_depth: whether to use depth estimation (default: False for faster processing)
        """
        global USE_DEPTH
        USE_DEPTH = use_depth
    ...
    def read_meta(self):
        ...
        if self.split == 'train':
            ...
            # Skip sparse depth generation and use empty depth data
            print("Skipping sparse depth generation. Using empty depth data.")
            self.depth_gts = [{'coord': np.array([]), 'depth': np.array([]), 'error': np.array([])} for _ in range(n_train)]

...
To run FrugalNeRF on Kaggle, use the following script in a Kaggle notebook. This script clones the repository, installs dependencies, sets up the environment, and runs training on the LLFF dataset.
...
!python train.py --config configs/llff_default_2v.txt --datadir ./data/nerf_llff_data/room --batch_size 4096 --train_frame_num 0 1 4 10  --test_frame_num 28 --n_iters 2000 --nSamples 4096 --downsample_train 8.0 --downsample_test 2.0 --vis_every 1000 --N_vis 5

# Training với 2 views (2v)
# !python train.py --config configs/llff_default_2v.txt --datadir ./data/nerf_llff_data/fern --batch_size 4096 --train_frame_num 6 13 --test_frame_num 0 8 16 --n_iters 2000 --nSamples 4096 --downsample_train 8.0 --downsample_test 2.0 --vis_every 1000 --N_vis 5

# Training với 3 views (3v) - uncomment để chạy
# !python train.py --config configs/llff_default_3v.txt --datadir ./data/nerf_llff_data/fern --batch_size 4096 --train_frame_num 5 10 14 --test_frame_num 0 8 16 24 32 --n_iters 2000 --nSamples 4096 --downsample_train 8.0 --downsample_test 2.0 --vis_every 1000 --N_vis 5

# Training với 4 views (4v) - uncomment để chạy
# !python train.py --config configs/llff_default_4v.txt --datadir ./data/nerf_llff_data/fern --batch_size 4096 --train_frame_num 4 7 12 15 --test_frame_num 0 8 16 24 32 40 --n_iters 2000 --nSamples 4096 --downsample_train 8.0 --downsample_test 2.0 --vis_every 1000 --N_vis 5
```

Note: Ensure you have the LLFF dataset uploaded to Kaggle as input data at `/kaggle/input/llff-dataset-full`.

## MapReduce Image Processing

This repository also includes a MapReduce component for parallel processing of images and data related to FrugalNeRF. The `mapreduce-image` directory (located at the root level) contains a Java-based Hadoop MapReduce application designed for distributed computing tasks such as:

- Image conversion and preprocessing
- Depth estimation
- Pose processing
- Ray generation
- Metadata parsing and output reading

This component is useful for handling large-scale datasets in a distributed environment, complementing the core FrugalNeRF training pipeline.

### Building and Running the MapReduce Jobs

To build the MapReduce application:

```bash
cd ../mapreduce-image  # From FrugalNeRF directory
mvn clean package
```

To run the jobs on a Hadoop cluster:

```bash
hadoop jar target/mapreduce-image-1.0-SNAPSHOT.jar org.mapreduce.Main [input_path] [output_path]
```

Ensure you have Java (JDK 8+) and Hadoop installed. This component is optional and intended for users working with large datasets requiring distributed processing.

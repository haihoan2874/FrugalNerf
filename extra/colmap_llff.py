...
def pipeline(scene, base_path, n_views):
    ...
    os.chdir(scene_path)
    view_path_full = os.path.join(scene_path, view_path)
    if os.path.exists(view_path_full):
        import shutil
        shutil.rmtree(view_path_full)
    os.makedirs(view_path_full)
    os.chdir(view_path_full)
...
for scene in ['fern', 'flower', 'fortress',  'horns',  'leaves',  'orchids',  'room',  'trex']:# ['bonsai', 'counter', 'garden', 'kitchen', 'room', 'stump']:
    pipeline(scene, base_path = './data/nerf_llff_data/', n_views = 2)  # using relative path
